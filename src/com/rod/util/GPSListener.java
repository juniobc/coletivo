...
public class GPSListener implements android.location.LocationListener {
	...
	public void setGPSParams(Long intervalo_tempo){
		...
		if(Preferencias.getBoolean(activity, "com.rod.coletivo", "gps")){
	...
	public boolean ativo(){
		if(!locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER) && !locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)){
			...
			a.setMessage(" GPS desativado, deseja ativar?");
			a.setCancelable(false);
			a.setButton(Alert.BUTTON_POSITIVE,"Sim", new DialogInterface.OnClickListener() {
				public void onClick(final DialogInterface dialog, final int id) {
					activity.startActivity(new Intent(Settings.ACTION_LOCATION_SOURCE_SETTINGS));
				}
			});
			a.setButton(Alert.BUTTON_NEGATIVE,"Não", new DialogInterface.OnClickListener() {
				public void onClick(final DialogInterface dialog, final int id) {
					dialog.cancel();
				}
			});
